 ...
 */
/**
 * @author liupengfei 此过滤器是自定义认证过滤器，只对原始的doFilter进行了重写，只适用于当前系统
 */
...
public class MyCasAuthenticationFilter extends AbstractCasFilter {
     ...
     * ajax请求
     */
    private final String AJAXREQUEST = "ajax";
    ...
    public final void doFilter(final ServletRequest servletRequest, final ServletResponse servletResponse, final FilterChain filterChain)
        ...
        // 只拦截主页和不带.的请求，因为系统功能接口都不带.所以这个东西具有定制性，不适用于其他系统
        final String requestUrl = request.getRequestURI();
        log.info("请求地址：" + requestUrl);
        if (requestUrl.indexOf(".") != -1 && requestUrl.indexOf("index.html") == -1) {
            filterChain.doFilter(request, response);
            return;
        }
        ...
        final String urlToRedirectTo = CommonUtils.constructRedirectUrl(this.casServerLoginUrl, getServiceParameterName(), modifiedServiceUrl,
                this.renew, this.gateway);
        if (AJAXREQUEST.equals(request.getHeader("requestType"))) {
            response.setStatus(401);
            log.info("未登录");
            ResultBean resultBean = ResultBeanFactory.getResultBeanFail(ErrorCode.CODE_AUTH_FAIL, ErrorCode.DESC_AUTH_FAIL);
            resultBean.setData(urlToRedirectTo.substring(0, urlToRedirectTo.indexOf("?")));
